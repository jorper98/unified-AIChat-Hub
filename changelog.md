...
## v0.4.13
- Strip internal XML tags (`<web_search_context>`, `<snippet>`, `<sources>`, `<source>`, `<answer>`) from AI responses to prevent them from leaking into the chat UI.
- Strip `[Generated by: ...]` model attribution tags from AI responses.
- Applied sanitization to both the main response path and the Perplexity fallback path.
