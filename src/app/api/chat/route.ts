...
export async function POST(request: NextRequest) {
  try {
    ...
    // Strip internal XML tags that may leak from web search context
    aiTextOutput = aiTextOutput
      .replace(/<\/?web_search_context>/g, '')
      .replace(/<\/?snippet[^>]*>/g, '')
      .replace(/<\/?sources>/g, '')
      .replace(/<source[^>]*\/?>/g, '')
      .replace(/<\/?answer>/g, '')
    ...
    aiTextOutput = aiTextOutput.replace(/\[Generated by:[^\]]*\]/g, '').trim();
    ...
    if (!bypassRouter && !perplexityUsage && isUncertainResponse(aiTextOutput)) {
      ...
      if (perplexityResult) {
        aiTextOutput = perplexityResult.answer;
        // Strip internal XML tags and model attribution from fallback response
        aiTextOutput = aiTextOutput
          .replace(/<\/?web_search_context>/g, '')
          .replace(/<\/?snippet[^>]*>/g, '')
          .replace(/<\/?sources>/g, '')
